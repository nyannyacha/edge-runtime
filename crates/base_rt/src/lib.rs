...
// are saturated.
pub static PRIMARY_WORKER_RT: Lazy<tokio::runtime::Runtime> = Lazy::new(|| {
    ...
    tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .thread_name("sb-primary-worker")
        .worker_threads(maybe_pool_size.unwrap_or(DEFAULT_PRIMARY_WORKER_POOL_SIZE))
        .build()
        .unwrap()
});

pub static USER_WORKER_RT: Lazy<tokio::runtime::Runtime> = Lazy::new(|| {
    ...
    tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .thread_name("sb-user-worker")
        .worker_threads(if cfg!(debug_assertions) {
