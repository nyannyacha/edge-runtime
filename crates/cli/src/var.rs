...
pub(super) fn resolve_base_rt_sched_var(matches: &ArgMatches) {
    runtime::sched::ENABLED.get_or_init(|| {
        matches
            .get_one("enable-rt-extra-scheduler")
            .copied()
            .unwrap_or_default()
    });

    runtime::sched::SAMPLE_SIZE.get_or_init(|| {
        matches
            .get_one("rt-extra-scheduler-sample-size")
            .copied()
            .unwrap()
    });

    runtime::sched::WORKER_THREAD_THRESHOLD.get_or_init(|| {
        matches
            .get_one("rt-extra-scheduler-worker-thread-threshold")
            .copied()
            .map(Duration::from_millis)
            .unwrap()
    });

    runtime::sched::BLOCKING_THREAD_THRESHOLD.get_or_init(|| {
        matches
            .get_one("rt-extra-scheduler-blocking-thread-threshold")
            .copied()
            .map(Duration::from_millis)
            .unwrap()
    });

    runtime::sched::BLOCKING_THREAD_RECHECK_INTERVAL.get_or_init(|| {
        matches
            .get_one("rt-extra-scheduler-blocking-thread-recheck-interval")
            .copied()
            .map(Duration::from_millis)
            .unwrap()
    });
