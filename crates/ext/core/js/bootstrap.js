...
globalThis.bootstrapSBEdge = (opts, extraCtx) => {
	...
	// NOTE(Nyannyacha): Put below inside `kind === 'user'` block if we have the plan to
	// support a shared array buffer across the isolates. But for now, we explicitly disabled the
	// shared buffer option between isolate globally in `deno_runtime.rs`, so this patch also
	// applies regardless of worker type.
