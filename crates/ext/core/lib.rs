...
use runtime_base::mem::WorkerHeapStatistics;
use serde::Serialize;
...
pub struct WorkerMetricSource {
    sender: IsolateHandleOpSender,
}

impl From<IsolateHandleOpSender> for WorkerMetricSource {
    fn from(sender: IsolateHandleOpSender) -> Self {
        Self { sender }
    }
}
...
impl RuntimeMetricSource {
    ...
    async fn get_heap_statistics(&mut self) -> RuntimeHeapStatistics {
        fn interrupt_fn(isolate: &mut v8::Isolate, heap_tx: oneshot::Sender<WorkerHeapStatistics>) {
            ...
            if let Err(err) = heap_tx.send(worker_stats) {
        ...
        let request_heap_statistics_fn = |arg: Option<&mut WorkerMetricSource>| {
            ...
            if !source
                .sender
                .request_interrupt(move |isolate| interrupt_fn(isolate, tx))
            {
