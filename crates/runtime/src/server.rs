...
use runtime_base::event::WorkerEventWithMetadata;
...
impl Server {
    pub async fn new(builder: Builder) -> Result<Self, Error> {
        let Builder {
            addr,
            tls,
            main_service_path,
        ...
        let mut worker_events_tx = None::<mpsc::UnboundedSender<WorkerEventWithMetadata>>;
        let termination_tokens =
            TerminationTokens::new(termination_token, event_worker_path.is_some());

        // Create Event Worker
        let event_worker_metric_src = if let Some(event_worker_path) = event_worker_path {
            let events_path = Path::new(&event_worker_path);
...
impl Builder {
    ...
    pub fn event_callback(&mut self, val: mpsc::Sender<ServerHealth>) -> &mut Self {
