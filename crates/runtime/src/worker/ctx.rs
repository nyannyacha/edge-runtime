...
use super::pool::{SupervisorPolicy, WorkerPoolPolicy};
use super::supervisor::{self, CPUTimerParam, CPUUsageMetrics};
use super::DuplexStreamEntry;
use super::{Worker, WorkerHandler};
...
pub fn create_supervisor(
    ...
    let isolate_handle_op_sender = worker_runtime.isolate_handle_op_sender();
    ...
    drop({
        let _rt_guard = runtime_base::rt::SUPERVISOR.enter();
            ...
            if let Some((session_tx, is_terminated, is_found)) = maybe_inspector_params {
                ...
                runtime_base::rt::SUPERVISOR
                    .spawn_blocking(move || {
                        ...
                        runtime_base::rt::SUPERVISOR.block_on(wait_inspector_disconnect_fut);
                    })
                    .await
                    .unwrap();
            }
...
pub async fn create_worker<Opt: Into<CreateWorkerArgs>>(
    ...
    let Some(worker_struct_ref) = worker.as_any().downcast_ref::<Worker>() else {
        return Err(anyhow!(
            "could not downcast worker handler to worker reference"
        ));
    };
