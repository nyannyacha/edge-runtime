...
use cpu_timer::{get_thread_time, CPUTimer, CPUTimerKind};
use ctor::ctor;
use deno_core::error::AnyError;
use deno_core::url::Url;
use deno_core::v8::{GCCallbackFlags, GCType, HeapStatistics, Isolate, Locker};
use deno_core::{
    located_script_name, serde_json, FastString, JsRuntime, ModuleId, ModuleLoader,
...
use runtime_base::isolate::{IsolateHandleOpQueue, IsolateHandleOpSender};
use runtime_base::mem::{MemCheckState, WorkerHeapStatistics};
...
use runtime_base::event::{EventMetadata, WorkerEventWithMetadata};
use sb_ai::sb_ai;
...
pub struct DenoRuntime<RuntimeContext = DefaultRuntimeContext> {
    ...
    waker: Arc<AtomicWaker>,
    isolate_handle_op_queue: IsolateHandleOpQueue,
...
impl<RuntimeContext> Drop for DenoRuntime<RuntimeContext> {
    fn drop(&mut self) {
        ...
        let locker =
            unsafe { Locker::new(std::mem::transmute::<&mut Isolate, &mut Isolate>(isolate)) };
    ...
    pub async fn new(
        ...
        let ret = unsafe {
            spawn_blocking_non_send(|| -> Result<Bootstrap, Error> {
                ...
                {
                    let _ = bootstrap.js_runtime.with_locker().execute_script(
                        located_script_name!(),
                        format!(
        ...
        let mut locker = unsafe { js_runtime.with_locker() };

        {
            ...
            if conf.is_main_worker() || is_user_worker {
        ...
        if is_user_worker {
            drop(runtime_base::rt::SUPERVISOR.spawn({
        ...
        // Be careful: if the locker goes out of scope, it's like calling `v8::Isolate::Exit()`.
        drop(locker);
        ...
        Ok(Self {
            ...
            mem_check,
            waker: Arc::default(),
            isolate_handle_op_queue: IsolateHandleOpQueue::default(),
...
impl<RuntimeContext> DenoRuntime<RuntimeContext> {
    ...
    pub fn isolate_handle_op_sender(&self) -> IsolateHandleOpSender {
        self.isolate_handle_op_queue.sender()
    }
    ...
    pub async fn run(
        ...
        let mut mod_result_rx = unsafe {
            ...
            if self.inspector().is_some() {
                let can_continue = spawn_blocking_non_send(|| {
                    ...
                    {
                        let _guard =
                            scopeguard::guard(locker.is_found_inspector_session.clone(), |v| {
                                v.raise();
                            });
            ...
            self.assert_isolate_not_locked();
            let mut locker = self.js_runtime.with_locker();

            with_cpu_metrics_guard(
                &maybe_cpu_usage_metrics_tx,
                &mut accumulated_cpu_time_ns,
                || locker.mod_evaluate(main_module_id),
            )
        };
    ...
    fn run_event_loop<'l>(
        ...
        poll_fn(move |cx| {
            ...
            self.assert_isolate_not_locked();
            let mut this = unsafe { self.with_locker() };
            let mut isolate_handle_op_dispatched = false;

            while let Poll::Ready(Some(op)) = this.isolate_handle_op_queue.rx.poll_recv(cx) {
                op.dispatch(this.js_runtime.v8_isolate());
                isolate_handle_op_dispatched = true;
            }
            ...
            let need_pool_event_loop = !is_user_worker || woked || isolate_handle_op_dispatched;
        ...
        C: FnOnce(MemCheckState) + Send + 'static,
    {
        ...
        drop(runtime_base::rt::SUPERVISOR.spawn(async move {
            tokio::select! {
    ...
    async fn init_main_module(&mut self) -> Result<(), AnyError> {
        ...
        let ret = unsafe {
            spawn_blocking_non_send(|| {
                handle.block_on(async {
                    self.assert_isolate_not_locked();
                    let mut locker = self.with_locker();
                    let code = locker.main_module_code.take();
    ...
    fn assert_isolate_not_locked(&mut self) {
        assert!(!Locker::is_locked(self.js_runtime.v8_isolate()));
    }
}
...
trait JsRuntimeLockerGuard {
    ...
    unsafe fn with_locker<'l>(
        ...
        let locker = Locker::new(std::mem::transmute::<&mut Isolate, &mut Isolate>(
...
async unsafe fn spawn_blocking_non_send<F, R>(non_send_fn: F) -> Result<R, tokio::task::JoinError>
where
    F: FnOnce() -> R,
    R: 'static,
{
    let disguised_fn = runtime_base::unsync::MaskValueAsSend { value: non_send_fn };

    let (mut scope, ..) = async_scoped::TokioScope::scope(|s| {
        s.spawn_blocking(move || runtime_base::unsync::MaskValueAsSend {
    ...
    let mut iter = stream
        .into_iter()
        .map(|it| it.map(runtime_base::unsync::MaskValueAsSend::into_inner));
...
fn get_cpu_metrics_guard<'l>(
    ...
    send_cpu_metrics_fn(CPUUsageMetrics::Enter(
        current_thread_id,
        CPUTimer::get_or_init(CPUTimerKind::Primary).unwrap(),
    ));
...
mod test {
    ...
    async fn test_eszip_with_source_file() {
        ...
        let main_mod_ev = rt.js_runtime.mod_evaluate(main_module_id);
        let _ = rt
            .js_runtime
    ...
    async fn test_jsx_import_source() {
        ...
        let _main_mod_ev = main_rt.js_runtime.mod_evaluate(main_module_id);
        let _ = main_rt
            .js_runtime
            .run_event_loop(PollEventLoopOptions::default())
            .await;
    ...
    async fn test_mem_check_above_limit(
        ...
        let isolate_handle_op_sender = user_rt.isolate_handle_op_sender();

        user_rt.add_memory_limit_callback(move |_| {
            assert!(isolate_handle_op_sender.terminate_execution());
